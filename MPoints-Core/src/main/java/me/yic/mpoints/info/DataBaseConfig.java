/*
 *  This file (DataBaseConfig.java) is a part of project MPoints
...
public class DataBaseConfig {
            ...
            try {
                ...
                if (getStorageType() == 0 || getStorageType() == 1) {
                    ...
                    try {
                        ...
                        EnableConnectionPool = false;
                        MPoints.getInstance().logger("connection-pool-unsupport", 1, null);
                    }
                }
            } catch (ClassNotFoundException e) {
                MPoints.getInstance().logger("未找到 'org.slf4j.Logger'", 1, null);
                EnableConnectionPool = false;
            }

            if (!EnableConnectionPool){
                MPoints.getInstance().logger("连接池未启用", 0, null);
            }
        }
    }
    ...
    public void loggersysmess(String tag) {
        ...
        switch (getStorageType()) {
            case 1:
                MPoints.getInstance().logger(null, 0, mess.replace("%type%", "SQLite"));
                break;
            case 2:
                MPoints.getInstance().logger(null, 0, mess.replace("%type%", "MySQL"));
                break;
            case 3:
                MPoints.getInstance().logger(null, 0, mess.replace("%type%", "MariaDB"));
                break;
        }
    }
